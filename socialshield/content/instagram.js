...
(function () {
  ...
  const SS_Instagram = {
    ...
    async runPrivacyScan() {
      ...
      // Lấy CẢ <header> (không chỉ section đầu) để bio không bị bỏ sót khi
      // apiBio rỗng (API fail). <header> chỉ chứa profile header — không gồm
      // suggested/feed nên không gây false-positive như khi quét toàn main.
      const bioSection = document.querySelector('main header') ||
